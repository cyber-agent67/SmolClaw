...
def ensure_connected_page(url: str, headless: bool = False) -> str:
    ...
    try:
        ...
        from smolclaw.agent.tools.ToolRegistry import get_browser_snapshot, set_browser_url
...
class SmolhandRunner:
    ...
    def _build_system_prompt(self) -> str:
        ...
        # Inject tool instructions from Prompts.json when available
        from smolclaw.config import load_tool_prompts

        tool_instructions: list[str] = []
        for tool_name in ["browser", "q_learning"]:
            prompts = load_tool_prompts(tool_name)
            if prompts:
                tool_instructions.append(json.dumps(prompts, indent=2))
